...
class AvatarLoadDialog(tk.Toplevel):
    def __init__(
        ...
        self.on_avatar_loaded = on_avatar_loaded  
        self.configure(bg=bg_color)
...
class PlayerView:
    def __init__(self, root, master_app):
        ...
        self.player_outline = "#4CAF50"  
        self.npc_outline = "#FFC107"  
        self.enemy_outline = "#F44336"  
...
class DnDMapMaster:
    ...
    def _configure_styles(self):
        ...
        thumb_color = lighten_color(SCROLLBAR_BG, 0.2)  
    ...
    def _fit_map_to_canvas(self):
        ...
        canvas_width = self.canvas.winfo_width() - 220  
    ...
    def redraw_map(self, delete_objects="all"):
        ...
        if isinstance(delete_objects, str):
            delete_objects = [delete_objects]

        def should_redraw(tag):
            return "all" in delete_objects or tag in delete_objects

        if self.map_photo:
            
            if should_redraw("map"):
            ...
            for tag in delete_objects:
                if tag.startswith("find_"):
                    self.canvas.delete(tag)
                    find_id = f"find_{tag.split('_')[-1]}"
                    if find_id in self.finds:
                        self._draw_find(self.finds[find_id])

        
        if should_redraw("ruler"):
            self.canvas.delete("ruler")
            self._draw_ruler()
        ...
        if hasattr(self, "player_view") and self.player_view:
            try:
                ...
                print(f"player_view redraw error: {e}")
